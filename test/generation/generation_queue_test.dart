...
void main() {
  ...
  group('batch & priority (§6.1)', () {
    test('startBatch enqueues all notGenerated words and consumes to done', () async {
    ...
    test('immediate task runs before remaining batch words (priority insertion)', () async {
      ...
      expect((await words(setId)).every((w) => w.status == WordStatus.done.name), isTrue);
    });
  ...
  group('pause / cancel / retry (§6)', () {
    test('pause stops dispatch, in-flight settle, resume continues', () async {
      ...
      g2.complete();
      await waitFor(() async =>
          (await words(setId)).take(2).every((w) => w.status == WordStatus.done.name));
      final ws = await words(setId);
      expect(ws[0].status, WordStatus.done.name);
    ...
    test('cancelWordSet resets its queued words, other sets unaffected', () async {
      await repo.saveSettings(const AppSettings(concurrency: 2));
      ...
      g2.complete();
      await waitFor(() async =>
          (await words(setIdA)).take(2).every((w) => w.status == WordStatus.done.name));
      await waitFor(() async => await q.pendingCount == 0);
      ...
      // B 词集不受影响
      expect((await words(setIdB)).every((w) => w.status == WordStatus.done.name), isTrue);
    });
  ...
  group('restart resume (§6.1)', () {
    test('pending tasks resume after restart (interrupted generating retried)', () async {
