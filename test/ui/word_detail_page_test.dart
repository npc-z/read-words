...
WordMaterial sampleMaterial() {
  final senses = [
    Sense(
      pos: 'v.',
      meaning: 'to move using your legs',
      examples: [
        Example(level: 1, en: 'I run every morning.', zh: '我每天早上跑步。'),
        Example(
          level: 1,
          en: 'The children ran across the park.',
          zh: '孩子们跑过公园。',
        ),
        Example(level: 1, en: 'She runs fast.', zh: '她跑得很快。'),
        Example(
          level: 2,
          en: 'He runs faster than anyone in class.',
          zh: '他跑得比班上任何人都快。',
        ),
        Example(
          level: 2,
          en: 'The dog that barked was running in circles.',
          zh: '那只叫的狗在绕圈跑。',
        ),
        Example(
          level: 2,
          en: 'Although it rained, we kept running along the river.',
          zh: '虽然下雨了,我们仍沿着河边跑。',
        ),
        Example(
          level: 3,
          en: 'Having run the length of the corridor, he collapsed against the door.',
          zh: '跑完走廊后,他瘫靠在门上。',
        ),
        Example(
          level: 3,
          en: 'The committee, having deliberated, concluded the proposal.',
          zh: '委员会审议后通过了提案。',
        ),
        Example(
          level: 3,
          en: 'While the storm raged, the children slept in the cabin.',
          zh: '暴风雨肆虐时,孩子们在小屋里睡着了。',
        ),
      ],
    ),
  ];
        ...
        [
          Sense(
            ...
            examples: [
              ...
              Example(
                level: 3,
                en: 'Having run the length of the corridor, he collapsed.',
                zh: '跑完走廊后,他瘫倒了。',
              ),
            ],
          ),
...
void main() {
  testWidgets('word detail renders study mode with level groups', (
    tester,
  ) async {
  ...
  testWidgets('opens in default view mode and display config from settings', (
    tester,
  ) async {
    final db = AppDatabase(NativeDatabase.memory());
    final repo = Repositories(db);
  ...
  testWidgets(
    'opening a notGenerated word triggers immediate generation (§6.1)',
    (tester) async {
      final db = AppDatabase(NativeDatabase.memory());
      final repo = Repositories(db);
      final setId = await repo.createWordSet('test');
      ...
      expect(
        (await repo.wordsInSet(setId)).single.status,
        WordStatus.done.name,
      );
      await db.close();
    },
  );
