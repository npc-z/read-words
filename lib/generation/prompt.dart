...
1. **例句总数恰好 9 条,三层配额固定(整个 JSON 全局统计,跨所有 senses)**:
   - **level 1 恰好 3 条**
   - **level 2 恰好 3 条**
   - **level 3 恰好 3 条**
   - 合计 9 条。**严禁**让每个义项各自配满 3 条(那是「义项数 × 3」,必然超过 9 条);义项多时,9 条配额在义项间分配,部分义项只分到 1~2 条。
2. 例句按义项分布:在 9 条配额内,主要义项分到更多例句;至少 7 句必须挂在 senses 的 examples 里,最多 2 句(习语化用法)可放入 unclassified_examples。
