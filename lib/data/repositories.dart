...
class Repositories {
  ...
  Future<void> setWordStatus(int wordId, WordStatus status) async {
    await (db.update(db.words)..where((w) => w.id.equals(wordId))).write(
      WordsCompanion(status: Value(status.name)),
    );
  }
  ...
  Future<int> addWords(int wordSetId, Iterable<String> headwords) async {
    final existing = await (db.select(
      db.words,
    )..where((w) => w.wordSetId.equals(wordSetId))).get();
  ...
  Future<WordMaterial?> materialFor(int wordId) async {
    return (db.select(
      db.wordMaterials,
    )..where((m) => m.wordId.equals(wordId))).getSingleOrNull();
  }

  Future<ReviewState?> reviewStateFor(int wordId) async {
    return (db.select(
      db.reviewStatesTable,
    )..where((r) => r.wordId.equals(wordId))).getSingleOrNull();
  }
  ...
  Future<List<DueReviewWord>> dueReviewWords(int wordSetId) async {
    final now = DateTime.now();
    final query =
        db.select(db.reviewStatesTable).join([
          innerJoin(
            db.words,
            db.words.id.equalsExp(db.reviewStatesTable.wordId),
          ),
        ])..where(
  ...
  Future<int> budgetUsedOn(String day) async {
    final row = await (db.select(
      db.budgetDays,
    )..where((r) => r.day.equals(day))).getSingleOrNull();
    return row?.count ?? 0;
  }
  ...
  Future<void> unclaimGenerationTask(int taskId) async {
    await (db.update(db.generationTasks)..where((t) => t.id.equals(taskId)))
  ...
  Future<void> resetInterruptedTasks() async {
    await db.transaction(() async {
      ...
      await (db.update(db.words)..where((w) => w.status.equals('generating')))
  ...
  Future<int> pendingGenerationTaskCount() async {
    return db
        .customSelect('SELECT COUNT(*) AS c FROM generation_tasks')
        .get()
        .then((rows) => rows.single.data['c'] as int);
  }
